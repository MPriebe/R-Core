...
gse            <- getGEO(accession.id, GSEMatrix = TRUE)  # Load the data
eset           <- GDS2eSet(gse, do.log2=TRUE)       # Convert into ExpressionSet Object
X              <- exprs(eset)               # Get Expression Data
geneNames        <- as.character(gse@dataTable@table$IDENTIFIER)
...
# BoxPlots with colours based on the disease states
